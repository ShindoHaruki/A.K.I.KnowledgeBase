...
### Updated
- 暗色ベースを維持しつつ、紫・毒緑・赤紫・金のアクセントを追加
- お知らせ、テクニック、参考資料、通常技組み合わせ、キャラ対の見出し色を分けるように調整
- カード上部にカテゴリ別のグラデーションラインを追加
- タグ表示を複数色でローテーションするように調整
- SA表の `SA1` / `SA2` / `SA3` をレベル別に色分け
- 背景に薄い毒霧風のレイヤーを追加

### Notes
- 情報量が多いページのため、本文・表のコントラストとカード余白は大きく変えずに維持
...
- テクニックに `9F詐欺` を追加
- テクニックに `10F詐欺` を追加
- テクニックに `しゃがみ中Pシミー` を追加
- キャラ対に `ラシード` 戦の対策を追加
- キャラ対に `マリーザ` 戦の対策を追加
- キャラ対に `マノン` 戦の対策を追加
- `index.html` に参考資料タブを追加
- `data/reference_tables.yml` を追加
